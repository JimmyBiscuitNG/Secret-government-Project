...
const file_name = "Resource/save.VIX"

var dir = DirAccess
var winmode = DisplayServer.WINDOW_MODE_EXCLUSIVE_FULLSCREEN
...
signal saved
func _process(delta):
	if not dir.dir_exists_absolute("Resource/"):
		dir.open("User://")
		dir.make_dir_absolute("Resource/")
	if DisplayServer.window_get_mode() == DisplayServer.WINDOW_MODE_EXCLUSIVE_FULLSCREEN:
		winmode = DisplayServer.WINDOW_MODE_EXCLUSIVE_FULLSCREEN
...
func save():
	...
	f.store_var(global.pwr)
	f.store_var(global.amt)
	f.store_var(global.tamt)
	f.store_var(global.tclk)
	f.store_var(global.Up1amt)
	f.store_var(global.Up1cst)
	f.store_var(global.Tool1Bought)
	f.store_var(global.Up1Multi)
	f.store_var(winmode)
	emit_signal("saved")
...
func _load():
	...
	if(f):
		global.pwr = f.get_var(global.pwr)
		global.amt = f.get_var(global.amt)
		global.tamt = f.get_var(global.tamt)
		global.tclk = f.get_var(global.tclk)
		global.Up1amt = f.get_var(global.Up1amt)
		global.Up1cst = f.get_var(global.Up1cst)
		global.Tool1Bought = f.get_var(global.Tool1Bought)
		global.Up1Multi = f.get_var(global.Up1Multi)
		DisplayServer.window_set_mode(f.get_var(winmode))
		print("loaded")
	emit_signal("loaded")
